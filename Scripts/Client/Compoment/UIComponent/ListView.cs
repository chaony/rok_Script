...
namespace Client
{
    public class ListView : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IEventSystemHandler
    {
        ...
        private ScrollBaseView parenScrollView;
            ...
            if (autoPlaySound)
            {
                ...
                if(delta>0&&delta<1)
                {
                    if (Mathf.Abs(delta - m_lastSoundDelta) >= soundInterval/itemList.Count)
                    {
                        ...
                        CoreUtils.audioService.PlayOneShot(soundAssetName, (tmpHandler) =>
                        {
                            m_audioHandler = tmpHandler;
                        });
                    }
                }
            }
        ...
        public void SetParentView(ScrollBaseView parent)
        {
            this.parenScrollView = parent;
        }
        ...
        public void OnValueChanged(Vector2 vec2)
        {
            ...
            {
                onValueChanged(this.GetContainerPos());
            }
        }
        ...
        public void MovePanelToPos(float pos)
        {
            SetContainerPos(pos);
            ShowContentAt(pos);
        }
        ...
        public void OnBeginDrag(PointerEventData eventData)
        {
            ...
            if (this.parentScrollRect != null && this.parentScrollEnable)
            {
                ...
                if(parenScrollView)
                {
                    this.parenScrollView.OnBeginDrag(eventData);
                    ScrollRect component = base.GetComponent<ScrollRect>();
                    if (component != null)
                    {
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            if (this.parentScrollRect != null && this.parentScrollEnable)
            {
                ...
                if(parenScrollView)
                {
                    parenScrollView.OnDrag(eventData);
                }
                if (parentListView)
                {
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            {
                CoreUtils.audioService.StopByHandler(m_audioHandler);
            }
            if (this.parentScrollRect != null && this.parentScrollEnable)
            {
                ...
                if(parenScrollView)
                {
                    parenScrollView.OnEndDrag(eventData);
                    ScrollRect component = base.GetComponent<ScrollRect>();
                    if (component != null)
                    {
                        component.vertical = this.isVertical;
