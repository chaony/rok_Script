...
public class ArabLayoutEditor : DecoratorEditor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        else
        {
            ...
            GUILayout.Space(30);
             UIDefaultValue uiDefaultValue = gui.GetComponent<UIDefaultValue>();
            if(!uiDefaultValue)
            {
                m_scale = GUILayout.Toggle(m_scale, "保存后缩放为0");
                m_alpha = GUILayout.Toggle(m_alpha, "保存后透明度为0");
            }
            else
            {
                m_scale = GUILayout.Toggle(uiDefaultValue.SetScaleOneInEditMode, "保存后缩放为0");
                m_alpha = GUILayout.Toggle(uiDefaultValue.SetCanvasGroupInEditMode, "保存后透明度为0");
            }

            if((m_scale||m_alpha)&&!uiDefaultValue)
            {
                uiDefaultValue = gui.gameObject.AddComponent<UIDefaultValue>();
            }
            if(uiDefaultValue)
            {
                uiDefaultValue.SetScaleOneInEditMode = m_scale;
                uiDefaultValue.SetCanvasGroupInEditMode = m_alpha;
            }
        }
    }
}